...
def account_status(acno): #fetch status
  conn = mysql.connector.connect(
      host='localhost', database='bankproject', user='root', password='')
  cursor = conn.cursor()
...
def deposit_amount(): #to deposit amount
    conn = mysql.connector.connect(
        host='localhost', database='bankproject', user='root', password='')
    cursor = conn.cursor()
...
def withdraw_amount():
    ...
    today = date.today()
    result = account_status(acno)
    if result[0] == 'active' and int(result[1])>=int(amount):    #to test whether demanded amount is greater than the account value or not
      ...
      print('\n\namount Withdrawn, enjoy !')

    else:
      print('\n\nClosed or Suspended Account Or Insufficient fund')
...
def daily_report():
   ...
   print('-'*100)
   plt.bar(today, len(records), color ='maroon',  
        width = 0.4) 
   plt.xlabel("Daily report") 
...
def monthly_report():
   ...
   today=input('enter month to search')
   cursor = conn.cursor()
...
def activate_account(): #meant for admin purpose
    conn = mysql.connector.connect(
        host='localhost', database='bankproject', user='root', password='')
    cursor = conn.cursor()
